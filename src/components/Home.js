...
const Home = () => {
  // added useNavigate hook to manually navigate to the /:topic/:name route
  const navigate = useNavigate();
  // added ref to the input elements to get their value
  const name = useRef();
  const topic = useRef();

  const handleSubmit = (e) => {
    e.preventDefault();
    let path = `/teachers/${topic.current.value}/${name.current.value}`;
    navigate(path);
  }
